...
app.config["SQLALCHEMY_DATABASE_URI"]="mysql+mysqldb://root:Tejasmcj$1@localhost/E_LEARN_MS"
db=SQLAlchemy(app)
...
def student():
    ...
    if request.method=="POST":
        ...
        sid=int(current_user.sid)
        student_query = text("DELETE FROM `enrollment` WHERE `enrollment`.`sid`=:student_sid AND `enrollment`.`cid`=:student_cid")
        student_param = {"student_sid":sid, "student_cid":cid}
        db.session.execute(student_query, student_param)
        db.session.commit()
        connection = mysql.connector.connect(host='localhost',
                                         database='E_LEARN_MS',
                                         user='root',
...
def tutor():
    trigdata=db.session.execute(text("SELECT * FROM `trig`"))
    studentdata=[]
    if request.method=="POST":
        cid=request.form.get("cid")
    ...
    if request.method=="POST":
        ...
        if student:
            for stud in enrollment:
                if cid==stud.cid:
                    marksupdate_query = text("INSERT INTO `assessment` (`sid`,`cid`,`asgmt1`,`asgmt2`,`asgmt3`) VALUES (:sid,:cid,:asgmt1,:asgmt2,:asgmt3) ON DUPLICATE KEY UPDATE `asgmt1`=:asgmt1,`asgmt2`=:asgmt2,`asgmt3`=:asgmt3;")
                    marksupdate_param = {"sid":sid, "cid":cid, "asgmt1":asgmt1, "asgmt2":asgmt2, "asgmt3":asgmt3}
                    db.session.execute(marksupdate_query,marksupdate_param)
                    db.session.commit()
    ...
    if request.method=="POST":
        ...
        for stud in enrollment:
            if cid==stud.cid:
                ...
                break
        if status:
            enrollment_text =  text("INSERT INTO `enrollment` (`enrollmentdate`,`sid`,`cid`) VALUES (:enrollmentdate,:sid,:cid);")
            enrollment_param = {"cid":cid, "sid":sid, "enrollmentdate":enrollmentdate}
            db.session.execute(enrollment_text, enrollment_param)
            db.session.commit()
...
def profileupdate():
    if request.method=="POST":
        ...
        student=Student.query.filter_by(email=email).first()
        if student and check_password_hash(student.password,password):
            profileupdate_query = text("UPDATE `student` SET `name`=:name,`phone`=:phone,`email`=:email WHERE `sid`=:sid;")
            profileupdate_param = {"name":name, "phone":phone, "email":email, "sid":sid}
            db.session.execute(profileupdate_query, profileupdate_param)
            db.session.commit()
